...
export function DespesaCard({ despesa, onClick, className }: DespesaCardProps) {
  ...
  return (
    <Card 
      className={`h-full transition-all duration-200 hover:shadow-md ${onClick ? 'cursor-pointer hover:scale-[1.02]' : ''} ${className || ''}`}
      onClick={onClick}
    >
      ...
      <CardContent className="pt-0 space-y-3">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <Badge 
            variant="outline" 
            className={`${getCategoriaColor(despesa.categoria)} font-medium`}
          >
            {despesa.categoria}
          </Badge>
        </div>
